...
function displayMovieInfo() {
  ...
  fetch(queryURL)
    ...
    .then(function (data) {
      const divMov = $("<div>");
      const ratingMov = $("<p>").text(`Rating: ${data.Ratings[0].Value}`);
      const releaseMov = $("<p>").text(`Release date: ${data.Released}`);
      const plotMov = $("<p>").text(`Plot: ${data.Plot}`);
      const posterMov = $("<img>").attr("src",data.Poster);
      divMov.append(ratingMov, releaseMov, plotMov, posterMov);
      $("#movies-view").prepend(divMov);
    });
}
